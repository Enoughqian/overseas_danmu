...
def get_sign(liveUuid):
    url = 'https://acs.h.alibaba.com/h5/mtop.alibaba.intl.live.getlive/1.0/'
    ...
    params = {
        ...
        'appKey': '24889839',
        't': now_time,
        'api': 'mtop.alibaba.intl.live.getLive',
        'v': '1.0',
        'type': 'jsonp',
        'H5Request': 'true',
        'dataType': 'jsonp',
        'callback': 'mtopjsonp1',
        'data': '{"uuid":"' + str(liveUuid) +'","platform":"pc","endpoint":"pc","referrer":"ScrollLivePC","cna":"0oSgH3eMog8CAd3ai0ozDuiR","language":"","currency":"USD"}',
    }
    ...
    c_data = '{"liveUuid":"' + liveUuid+ '","language":"","currency":"USD"}'
    pre_encode = d_token + "&" + str(time.time()*1000).split(".")[0] + "&" + g + "&" + c_data
...
def get_data(liveUuid, live_id):
    url = 'https://acs.h.alibaba.com/h5/mtop.alibaba.intl.live.getlivecomment/1.0/'
    
    try:
        params_sign, params_t, params_cookies = get_sign(liveUuid)
        
        print(params_cookies)
        print(params_sign)
        print(params_t)
        print("--------------")
        params = {
            ...
            't': str(params_t),
            'sign': params_sign,
            'api': 'mtop.alibaba.intl.live.getLiveComment',
            'v': '1.0',
            'type': 'jsonp',
            'H5Request': 'true',
            'dataType': 'jsonp',
            'callback': 'mtopjsonp5',
            'data': '{"uuid":"'+ str(liveUuid) +'","language":"","currency":"USD"}'
        }
        ...
        response = requests.get(url, params=params, headers=headers, cookies=params_cookies)
        print("二次请求")
        print(response.text)
